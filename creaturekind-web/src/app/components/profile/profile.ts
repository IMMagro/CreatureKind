...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  private fetchServerStats() {
    this.http.get<any>('http://192.168.10.139:8000/api/server-stats').subscribe({
      next: (stats) => {
  ...
  rechargeCredits() {
    this.http.post<any>('http://192.168.10.139:8000/api/recharge', { email: this.user.email }).subscribe({
      next: (res) => {
